...
function AEkeyTime(int frameNum){
    return (float(frameNum)/float(counter)) * (float(counter)/float(fps));
}

function AEkeyPos(int spriteNum, int frameNum){
...
function AEkeyRot(int spriteNum, int frameNum){
     ...
     float lower, upper, centerNum;
         ...
         if(frameNum%smoothNum==0||frameNum==0||frameNum==counter-1){
            obj1 += "\t\t" + "r.setValueAtTime(" + AEkeyTime(frameNum) + ", " + centerNum +");" + "\r";
         }
}

function AEeffects(){
         obj1 += "\t" + "var myEffect = solid.property(\"Effects\").addProperty(\"Fast Blur\")(\"Blurriness\").setValue(61);";
}

//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

function AEkeysBegin() {
    //dataAE = new Data();
    //dataAE.beginSave();
    obj1 = "";
